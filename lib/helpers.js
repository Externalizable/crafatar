...
// decides whether to get a +type+ image for +userId+ from disk, download it, or use userId hash
// callback: error, status, hash, slim
// for status, see response.js
exp.get_image_hash = function(rid, userId, type, callback) {
  if (userId.length >= 16) {
    callback(null, 1, userId, 0)
    return;
  }
...
exp.get_render = function(rid, userId, scale, overlay, slim_overwrite, body, callback) {
  exp.get_skin(rid, userId, function(err, skin_hash, status, img, slim) {
    if (!skin_hash) {
      callback(err, status, skin_hash, null);
      return;
    }
    var renderpath = path.join(config.directories.renders, [skin_hash, scale, get_type(overlay, body), slim_overwrite || slim ? "s" : "t"].join("-") + ".png");
    fs.access(renderpath, function(fs_err) {
      if (!fs_err) {
        ...
        renders.draw_model(rid, img, scale, overlay, body, slim_overwrite || slim || userId.toLowerCase() === "mhf_alex", function(draw_err, drawn_img) {
          if (draw_err) {
