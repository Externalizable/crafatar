...
module.exports = function(req, callback) {
  ...
  var slim_overwrite = Object.prototype.hasOwnProperty.call(req.url.query, "slim");
  ...
  if (scale < config.renders.min_scale || scale > config.renders.max_scale) {
    ...
    return;
  } else if (!helpers.id_valid(userId) && userId.length < 16) {
    callback({
      status: -2,
      body: "Invalid UUID/Hash"
    });
    return;
  }
  ...
  try {
    helpers.get_render(rid, userId, scale, overlay, slim_overwrite, body, function(err, status, hash, image) {
      if (err) {
        if (err.code === "ENOENT") {
          // no such file
